...
N_f=$2
epochs_og=126
expname="CRT-C$C_f-$N_f[$minlr_og, $maxlr_og]_Batch64_${epochs_og}eps"
...
CUDA_VISIBLE_DEVICES=$(nvidia-smi --query-gpu=memory.free --format=csv,nounits,noheader | nl -v 0 | sort -nrk 2 | cut -f 1 | head -n 1 | xargs)\
 ...
 --finetune-final=False --name-ftF="$name_ftF" --minL-ftF=$ftF_L --maxL-ft=$ftF_R --stepL-ftF=$ftF_S --prefix-ftF="$prefix_ftF"